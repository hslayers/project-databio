...
import water_bodies from 'water_bodies';
import parcels_with_id from 'parcels_with_id';
import erosion_zones from 'erosion_zones';
import parcels_with_CTVDPB from 'parcels_with_CTVDPB';
...
module.controller('Main', ['$scope', '$compile', '$element', 'Core', 'hs.map.service', 'config', '$rootScope', 'hs.utils.service', '$sce', 'hs.sentinel.service', 'gettext',
    function ($scope, $compile, $element, Core, hs_map, config, $rootScope, utils, $sce, sentinel_service, gettext) {
        ...
        $rootScope.$on('cesiummap.resized', function () {
            if(typeof hsCesium == 'undefined') return;
            synced_providers.forEach(function (provider) {
        ...
        $scope.$on("scope_loaded", function (event, args) {
            if (args == 'Sidebar') {
                var el = angular.element('<div hs.sentinel.directive hs.draggable ng-controller="hs.sentinel.controller" ng-if="Core.exists(\'hs.sentinel.controller\')" ng-show="Core.panelVisible(\'sentinel\', this)"></div>')[0];
                document.querySelector('#panelplace').appendChild(el);
                $compile(el)($scope);

                var toolbar_button = angular.element('<div hs.sentinel.toolbar></div>')[0];
